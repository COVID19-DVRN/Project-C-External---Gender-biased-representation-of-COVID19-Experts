...
df_entity_to_annotated_race_gender_expertise = pd.read_csv(f"../outputs/data/08_01_02_entity_race_gender_expertise_news_count_{selection_type}.csv", na_filter=False)
...
input_basename=f"comention_network_with_metadata_{selection_type}"
input_full_fname=f"{input_dir}/05_01_02_{input_basename}.edgelist"
...
df_individuals_inside_network.to_csv(f"../outputs/data/{output_code}_individuals_in_comention_network_{selection_type}.csv", index=False)
...
plt.savefig(f"../figures/{output_code}_degree_dist_with_powerlaw_fit_{selection_type}.png", dpi =300)
...
value_label = "degree"
savefig_dir = f"../figures/{output_code}_histogram_{value_label}_{selection_type}.png"
...
for attribute in ["urm","pronoun"]:
	...
	plt.tight_layout()
	savefig_dir = f"../figures/{output_code}_degree_distribution_by_attribute_level_{attribute}_{selection_type}.png"
...
ax.legend()
plt.tight_layout()
plt.savefig(f"../figures/{output_code}_tail_glass_ceiling_{selection_type}.png",dpi=150)
plt.show()
		...
		if normalization == "all_normalized":
			...
			for (i, j), z in np.ndenumerate(cell_text):
			    ax.text(j+0.5, i+0.5, '{:0.0f}'.format(z*100)+"%", ha='center', va='center',
			    	bbox=dict(boxstyle='round', facecolor='white', edgecolor='0.3'))
		savefig_dir = f"../figures/{output_code}_assortativity_attributes_{attribute}_{normalization}_comention_network_{selection_type}.png"
		plt.savefig(savefig_dir, dpi=150)
		plt.show()
#attribute = "race"